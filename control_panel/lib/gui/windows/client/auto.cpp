...
namespace GUI {
    ...
    namespace Windows {
        ...
        void Auto::start_sending_cb(std::stop_token st, Auto& self) {
            ...
            self.shm->active_devices->at(self.index).measurement->clear();
            self.shm->cmd.send(
                BLE_Client::StateMachines::Connection::Events::write_body_composition_feature{
                    self.index,
            ...
            while(st.stop_requested() == false) {
                const auto rx_payload { self.shm->active_devices->at(self.index).measurement->read_for(std::chrono::milliseconds(10'000)) };
